        ...
        async function create_(userID, userInfo) {
                ...
                const queue = new Promise(async function (resolve_, reject_) {
                        try {
                                ...
                                if (!userInfo) {
                                        try {
                                                const fetchedInfo = await api.getUserInfo(userID);
                                                userInfo = fetchedInfo ? fetchedInfo[userID] : null;
                                        } catch (err) {
                                                console.log(`[usersData] getUserInfo failed for userID ${userID}:`, err.message);
                                                userInfo = null;
                                        }
                                }
                                
                                if (!userInfo || !userInfo.name) {
                                        console.log(`[usersData] Missing or incomplete userInfo for userID ${userID}, using fallback values`);
                                        userInfo = {
                                                name: "Facebook User",
                                                gender: null,
                                                vanity: null
                        ...
                        taskQueue.push(async function () {
                                try {
                                        ...
                                        if (!updateInfoUser) {
                                                try {
                                                        const fetchedInfo = await api.getUserInfo(userID);
                                                        updateInfoUser = fetchedInfo ? fetchedInfo[userID] : null;
                                                } catch (err) {
                                                        console.log(`[usersData] getUserInfo failed for userID ${userID} in refreshInfo:`, err.message);
                                                        updateInfoUser = null;
                                                }
                                        }
                                        
                                        if (!updateInfoUser || !updateInfoUser.name) {
                                                console.log(`[usersData] Cannot refresh userInfo for userID ${userID}, keeping existing data`);
